<x-action-section>
    ...
    <x-slot name="content">
        ...
        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('Quando a autenticação de dois fatores está habilitada, você será solicitado a inserir um token seguro e aleatório durante a autenticação. Você pode obter este token do aplicativo Google Authenticator em seu telefone.') }}
            </p>
        </div>
            ...
            @if ($showingQrCode)
                <div class="mt-4 max-w-xl text-sm text-gray-600">
                    <p class="font-semibold">
                        @if ($showingConfirmation)
                            {{ __('Para concluir a ativação da autenticação de dois fatores, escaneie o QR code a seguir usando o aplicativo autenticador do seu telefone ou insira a chave de configuração e forneça o código OTP gerado.') }}
                        @else
                            {{ __('A autenticação de dois fatores está agora ativada. Escaneie o QR code abaixo usando o aplicativo autenticador do seu telefone ou insira a chave de configuração.') }}
                        @endif
                    </p>
                </div>
